...
class L1ClassificationModel(MLPModel, ModelIOKeysMixin):
    def __init__(self, input_dim, nr_classes, hidden_dims, batch_norm=None, dropout=None, activation='relu', lam=0.1):
        super().__init__(input_dim, nr_classes, hidden_dims,
                         batch_norm=batch_norm, dropout=dropout, activation=activation)
        self.loss = nn.CrossEntropyLoss()  # Assuming nr_classes > 2 for multi-class classification
        self.lam = lam  # L1 regularization coefficient

    def forward(self, feed_dict):
        logits = super().forward(self._get_input(feed_dict))
        if self.training:
            loss = self.loss(logits, self._get_label(feed_dict))
            # Compute the L1 regularization term for all weights
    ...
    def _compose_output(self, logits):
        prob = nn.functional.softmax(logits, dim=1)  # Convert logits to probabilities for multi-class
        _, pred = torch.max(prob, dim=1)  # Predicted class
        return dict(prob=prob, pred=pred, logits=logits)
